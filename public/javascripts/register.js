...
function validateInputs() {
    ...
    if (!phoneNumber)
        ...
        $("#error4").html("")
    gender ? $("#error5").html("") : $("#error5").html("الزامی")
    if (!pass1)
        $("#error6").html("الزامی")
    else if (!(/^(?=.*[A-Za-z])(?=.*\d)[A-Za-z\d]{8,}$/).test(pass1))
        $("#error6").html("پسوورد باید حداقل 8 کاراکتر شامل حرف و عدد باشد")
    else
        $("#error6").html("")
    if (!pass2)
        $("#error7").html("الزامی")
    ...
    return (!$("#error1").html() && !$("#error2").html() && !$("#error3").html() && !$("#error4").html() && !$("#error5").html() && !$("#error6").html() && !$("#error7").html())
}